...
// --- HELPER: SYNTHETIC CATEGORIZATION ---
// This scans the title and description to guess the category since the DB doesn't have one
const getSyntheticCategory = (course: Course) => {
  const text = (course.title + " " + course.description).toLowerCase();
  
  if (text.includes('code') || text.includes('python') || text.includes('react') || text.includes('web') || text.includes('data') || text.includes('ai') || text.includes('tech')) return 'Tech & Coding';
  if (text.includes('business') || text.includes('money') || text.includes('marketing') || text.includes('sales') || text.includes('finance') || text.includes('invest')) return 'Business';
  if (text.includes('design') || text.includes('art') || text.includes('photo') || text.includes('music') || text.includes('draw') || text.includes('creative')) return 'Creative';
  if (text.includes('math') || text.includes('science') || text.includes('physics') || text.includes('history') || text.includes('language')) return 'Academics';
...
export const Courses: React.FC<CoursesProps> = ({ courses, onPurchase, onOpenReader }) => {
  ...
  const categories = ['All', 'Free', 'Tech & Coding', 'Business', 'Creative', 'Academics', 'General'];
  ...
  const handleDownloadPdf = (course: Course) => {
    ...
    const htmlContent = `
        ...
        <body>
            ...
            <div class="content">${course.content || '<p><i>No written content available for this course yet.</i></p>'}</div>
            <div class="footer">Generated by Grecko Learning Platform</div>
  ...
  // --- FILTERING LOGIC ---
  // 1. First, split Owned vs Not Owned
  const allOwned = courses.filter(c => c.isOwned);
  const allMarketplace = courses.filter(c => !c.isOwned);

  // 2. Filter Marketplace based on Search AND Category
  const filteredMarketplace = allMarketplace.filter(course => {
    // Search Check
    const matchesSearch = course.title.toLowerCase().includes(searchQuery.toLowerCase()) || 
                         course.author.toLowerCase().includes(searchQuery.toLowerCase());
    
    // Category Check
    const derivedCategory = getSyntheticCategory(course);
    let matchesCategory = true;

    if (activeCategory === 'All') matchesCategory = true;
    else if (activeCategory === 'Free') matchesCategory = course.price === 0;
    else matchesCategory = derivedCategory === activeCategory;

    return matchesSearch && matchesCategory;
  });

  // 3. Filter Owned based on Search only (Owned usually doesn't need category filtering as strictly)
  const filteredOwned = allOwned.filter(course => 
  ...
  return (
    <div className="space-y-8 pb-32 md:pb-20">
      <div className="flex flex-col gap-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Access your content and explore the marketplace</p>
          </div>
          <div className="w-full md:w-96 relative group z-10">
            <div className="relative flex items-center bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 focus-within:border-primary focus-within:ring-1 focus-within:ring-primary rounded-[28px] transition-all duration-200 ease-in-out shadow-sm hover:shadow-md">
              <Search className="w-5 h-5 text-slate-400 ml-5 shrink-0" />
              <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search titles or authors..." className="w-full bg-transparent text-slate-900 dark:text-white placeholder-slate-400 px-4 py-3.5 outline-none border-none focus:ring-0 text-sm rounded-[28px]" />
              {searchQuery && (<button onClick={() => setSearchQuery('')} className="mr-3 p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-zinc-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors"><X className="w-4 h-4" /></button>)}
            </div>
          </div>
        </div>
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all duration-200 ${
                activeCategory === cat
                  ...
                  : 'bg-white dark:bg-zinc-900 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-zinc-800 hover:border-blue-400'
              }`}
            >
              {cat}
            </button>
      ...
      {/* --- SECTION 1: MY LEARNING (OWNED) --- */}
      {filteredOwned.length > 0 && (
        <section className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <div className="p-1.5 bg-green-100 dark:bg-green-900/30 rounded-lg"><BookOpen className="w-4 h-4 text-green-600 dark:text-green-400" /></div> 
            My Learning
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
            {filteredOwned.map(course => (<CourseItem key={course.id} course={course} onDownloadPdf={handleDownloadPdf} onOpenReader={onOpenReader} onPurchase={handlePurchaseClick} />))}
          </div>
        </section>
      )}
      ...
      <section className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-zinc-800 pb-2">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">
            {searchQuery ? 'Search Results' : `Explore ${activeCategory === 'All' ? 'Marketplace' : activeCategory}`}
          </h2>
          <span className="text-xs text-slate-500">{filteredMarketplace.length} courses</span>
        </div>

        {filteredMarketplace.length === 0 && filteredOwned.length === 0 ? (
          <div className="text-center py-24 border border-dashed border-slate-200 dark:border-zinc-800 rounded-3xl bg-slate-50/50 dark:bg-zinc-900/50">
            ...
            <Button variant="ghost" className="mt-6" onClick={() => { setSearchQuery(''); setActiveCategory('All'); }}>Clear All Filters</Button>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
            {filteredMarketplace.map(course => (<CourseItem key={course.id} course={course} onDownloadPdf={handleDownloadPdf} onOpenReader={onOpenReader} onPurchase={handlePurchaseClick} />))}
          </div>
        )}
      </section>

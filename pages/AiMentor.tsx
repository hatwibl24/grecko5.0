...
const TypingIndicator = () => (
  <div className="flex items-center gap-1.5 p-4 bg-zinc-900 border border-zinc-800 rounded-2xl w-fit animate-in fade-in slide-in-from-bottom-2">
...
export const AiMentor: React.FC<AiMentorProps> = ({ user, assignments, academicGoals, quizResults, courses, onViewCourse }) => {
  ...
  useEffect(() => {
    ...
    shuffleSuggestions();
    const interval = setInterval(shuffleSuggestions, 8000);
    return () => clearInterval(interval);
  }, []);
    ...
    try {
      const payload = {
            ...
            context: {
                ...
                // OPTIMIZED: Only sending necessary course data
  ...
  return (
    <div className="flex flex-col h-full relative bg-black text-slate-200 font-sans selection:bg-blue-500/30 overflow-hidden pb-16 md:pb-0">
          ...
          {sessions.length === 0 ? <div className="text-center text-slate-500 text-sm mt-10">No past conversations</div> : sessions.map(session => (
            <div key={session.id} onClick={() => handleSelectChat(session)} className={`group flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${activeSessionId === session.id ? 'bg-zinc-800 text-white' : 'text-slate-400 hover:bg-zinc-800/50 hover:text-slate-200'}`}>
              ...
              <button
                ...
                className="p-2 text-red-500 hover:bg-red-500/10 rounded-lg transition-colors z-10 opacity-0 group-hover:opacity-100"
                title="Delete Chat"
              >
          ...
          <div className="max-w-3xl mx-auto space-y-6 pt-4">
            {currentMessages.map(msg => (
              <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                ...
                {/* Avatar */}
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-lg ${msg.role === 'user' ? 'bg-blue-600 text-white' : 'bg-zinc-800 border border-zinc-700 text-white'}`}>
                  {msg.role === 'user' ? <span className="font-bold text-xs">{user.name.charAt(0)}</span> : <AiIcon className="w-5 h-5 text-white" />}
                </div>

                {/* Message Bubble */}
                <div className={`flex-1 max-w-[85%] rounded-2xl p-3 md:p-5 leading-relaxed text-sm md:text-base break-words shadow-md ${
                    msg.role === 'user' 
                            ...
                            a: ({ href, children }: { href?: string; children?: React.ReactNode }) => {
                                if (href?.startsWith('course:')) {
                                    ...
                                    return (
                                        ...
                                        >
                                            <span className="underline decoration-blue-500/30 underline-offset-2">{children}</span>
                                        </button>
                                    );
                                }
      ...
      <div className="flex-none p-4 md:p-6 bg-black border-t border-zinc-900 z-30">
        <div className="max-w-3xl mx-auto pointer-events-auto">
          ...
          <form onSubmit={(e) => handleSend(e)} className="relative w-full">
            <div className="relative flex items-center bg-zinc-900 rounded-[26px] px-4 py-3 border border-zinc-800 transition-colors focus-within:border-zinc-700 focus-within:ring-1 focus-within:ring-zinc-700 shadow-xl">
              <input
                type="text"
                value={input}

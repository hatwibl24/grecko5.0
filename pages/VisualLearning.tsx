...
export const VisualLearning: React.FC<{ onNavigateToCourse: () => void }> = ({ onNavigateToCourse }) => {
  ...
  const fetchFeed = useCallback(async (query = "") => {
    ...
    try {
        ...
        // FIXED SEARCH: Now also filters Courses (Ads) by the query
        let courseQuery = supabase.from('courses').select('*').eq('is_published', true);
        if (query) {
            courseQuery = courseQuery.or(`title.ilike.%${query}%,description.ilike.%${query}%`);
        }
        ...
        // If searching, we just combine them. ConstructFeed is for random browse.

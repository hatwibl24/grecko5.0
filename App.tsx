...
export default function App() {
  ...
  const handlePurchaseCourse = async (cid: string) => { 
      ...
      // 1. Optimistic UI Update (Show "Owned" immediately)
      setCourses(c => c.map(x => x.id === cid ? { ...x, isOwned: true } : x));
      
      // 2. Don't Insert here! The Edge Function already inserted it during payment.
      // We just fetch the latest data to ensure everything is synced.
      fetchUserData(); 
  };
